...
class jisuan:
    ...
    def show(self):
        ...
        btnjian = tkinter.Button(text='一', command=lambda: self.presign('-'))
        ...
        # 特殊运算符号
        btndaoshu = tkinter.Button(text='1/x', command=lambda: self.special('1/x'))
        btndaoshu.place(x=210, y=75, width=45, height=35)

        btngenhao = tkinter.Button(text='√', command=lambda: self.special('√'))
        btngenhao.place(x=210, y=115, width=45, height=40)

        btnsqrt = tkinter.Button(text='㎡', command=lambda: self.special('m2'))
        btnsqrt.place(x=210, y=160, width=45, height=40)

        btnsqrt = tkinter.Button(text='+/-', command=lambda: self.special('+/-'))
        btnsqrt.place(x=160, y=75, width=45, height=35)

        btnsqrt = tkinter.Button(text='C', command=lambda: self.special('C'))
        btnsqrt.place(x=10, y=75, width=45, height=35)

        btndel = tkinter.Button(text='←', command=lambda: self.special('←'))
        btndel.place(x=110, y=75, width=45, height=35)

        btnclear = tkinter.Button(text='CE', command=lambda: self.special('CE'))
        btnclear.place(x=60, y=75, width=45, height=35)
